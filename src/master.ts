...
function handleConfig(c?: Config) {
    return new Promise<void>((resolve, reject) => {
        if (c) {
            ...
            if (err) return resolve(await askQuestions().catch(reject));//return reject(err);
                ...
                prompt.ask(`Enter absolute path of folder/file to backup: `).then((path) => {
                    ...
                    (function yn() {
                        prompt.ask(`More file/folder to backup [Y/N]? `).then(confirm => {
                            if (confirm.toLowerCase() === 'y') return getInput().then(resolve).catch(reject);
                            else if (confirm.toLowerCase() === 'n') return resolve();
                            yn();
                        });
                    })();
                })
                ...
                prompt.ask(`Enter absolute path of folder to store encrypted file: `).then((path) => {
                    ...
                    (function yn() {
                        prompt.ask(`More output destination [Y/N]? `).then(confirm => {
                            if (confirm.toLowerCase() === 'y') return getOutput().then(resolve).catch(reject);
                            else if (confirm.toLowerCase() === 'n') return resolve();
                            yn();
                        });
                    })();
            ...
            return new Promise(resolve => {
                (function yn() {
                    prompt.ask(`Enable watch mode [Y/N]? `).then(confirm => {
                        if (confirm.toLowerCase() === 'y') return ans.watch = 60, resolve();
                        else if (confirm.toLowerCase() === 'n') return resolve();
                        yn();
                    });
                })();
            ...
            return new Promise(resolve => {
                (function yn() {
                    prompt.ask(`Save your password [Y/N]? `).then(confirm => {
                        if (confirm.toLowerCase() === 'y') return resolve()
                        else if (confirm.toLowerCase() === 'n') return nosave = true, resolve();
                        yn();
                    });
                })();
            })
