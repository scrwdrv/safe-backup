...
cpc.onMaster('decrypt', async (req, res) => {
    ...
    try {
        ...
        }), key = crypto.privateDecrypt(privateKey, head.encryptedKey), input = PATH.parse(req.input), output = PATH.join(input.dir, input.name), extract = new archive.Extract();
    ...
    const inputStats = await new Promise((resolve) => fs.stat(req.input, (err, stats) => {
        if (err)
            return res(err);
        resolve(stats);
    })), isFile = inputStats.isFile(), l = req.output.length, fileName = formatPath(req.input.replace(/[\\*/!|:?<>]+/g, '-'), 255) + '.backup', outputs = req.output.map(p => { return fs.createWriteStream(PATH.join(p, fileName + '.temp')); }), writeStream = new stream_1.Writable({
    ...
    })).then(() => res(null, bytesLength, mods)).catch(res)), pack = new archive.Pack();
    ...
    try {
        if (isFile) {
            ...
            if (head) {
                ...
                const passwordHash = await keytar.getPassword('safe-backup', req.account), privateKeyDecipher = crypto.createDecipheriv('aes-256-gcm', hashPassword(passwordHash), head.encryptedPrivateKey.slice(-12)).setAuthTag(head.encryptedPrivateKey.slice(-28, -12)), privateKey = crypto.createPrivateKey({
                    key: Buffer.concat([privateKeyDecipher.update(head.encryptedPrivateKey.slice(0, -28)), privateKeyDecipher.final()]),
                    format: 'pem',
