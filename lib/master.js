...
(async function init() {
    try {
        ...
        if (newerVersion)
            log.warn(`safe-backup v${newerVersion} released, ${addcolor_1.default.yellow('`npm update -g safe-backup`')} to update`);
...
function prettyJSON(json) {
    ...
    return str.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, (match) => {
        ...
        if (/^"/.test(match))
            if (/:$/.test(match))
                cls = addcolor_1.default.code.fg.cyan + addcolor_1.default.code.style.bright;
            else
                cls = addcolor_1.default.code.fg.white + addcolor_1.default.code.style.bright;
        else
            cls = addcolor_1.default.code.fg.magenta + addcolor_1.default.code.style.bright;
