...
cpc.onMaster('saveLog', async (req, res) => {
            ...
            if (header.type === 'file')
                if (head.isFile)
                    stream
                        ...
                        .pipe(fs.createWriteStream(output, { mode: header.mode || 0o666 })).on('finish', () => utimes(output, (err) => {
                        if (err)
                            return next(err);
                        next();
                    }));
                    ...
                    mkdir(dirPath, dirName, (err) => {
                        ...
                        stream
                            .pipe(crypto.createDecipheriv('aes-256-ctr', key, hashMD5(header.name)))
                            .pipe(fs.createWriteStream(path, { mode: header.mode || 0o666 })).on('finish', () => utimes(path, (err) => {
                            if (err)
                                return next(err);
                            next();
                        }));
    ...
    const inputStats = await new Promise((resolve) => fs.stat(req.input, (err, stats) => {
        if (err)
            return res(err);
        resolve(stats);
    })), isFile = inputStats.isFile(), l = req.output.length, fileName = formatPath(req.input.replace(/[\\*/!|:?<>]+/g, '-'), 240) + '.bua', outputs = req.output.map(p => { return fs.createWriteStream(PATH.join(p, fileName + '.temp')); }), writeStream = new stream_1.Writable({
...
}).onMaster('plainBackup', async (req, res) => {
    let bytesLength = 0, mods = {
        file: [0, 0],
        directory: [0, 0]
    }, entries = {};
    const inputStats = await new Promise((resolve) => fs.stat(req.input, (err, stats) => {
        if (err)
            return res(err);
        resolve(stats);
    })), isFile = inputStats.isFile(), fileName = formatPath(req.input.replace(/[\\*/!|:?<>]+/g, '-'), 255), regs = req.ignore.map(str => {
        return regex.from(str);
    }), prefixLength = req.input.length;
    ...
    getEntry(req.input, (err) => {
        ...
        Promise.all(req.output.map(p => {
            return new Promise((resolve, reject) => recursiveCheck(PATH.join(p, fileName), (err) => {
                if (err)
                    return reject(err);
                resolve();
            }));
    ...
    function recursiveCheck(path, cb, prefixLength = path.length) {
        const name = normalizePath(path.slice(prefixLength)), n = name.split('/');
        for (let i = regs.length; i--;)
            if (regs[i].test(name) || n.indexOfRegex(regs[i]) > -1)
                return cb();
        if (entries[name] === 0)
            ...
            fs.stat(path, (err, stats) => {
                if (err)
                    return cb(err);
                if (stats.isDirectory())
                    recursiveRmdir(path).then(cb).catch(cb);
                else
                    fs.unlink(path, cb), mods.file[1]++;
            });
    }
    function getEntry(path, cb) {
        const name = normalizePath(path.slice(prefixLength)), n = name.split('/');
                ...
                promises.push(p = p.then(() => new Promise((resolve, reject) => {
                    ...
                    fs.stat(p, (err, stats) => {
                        if (err)
                            ...
                            recursiveRmdir(p).then(resolve).catch(reject);
                        else
                            fs.unlink(p, err => {
                                if (err)
                                    return reject(err);
    ...
    function mkdirs(path, inputStats, cb) {
        Promise.all(req.output.map(p => {
            p = PATH.join(p, fileName, path.slice(prefixLength));
            return new Promise((resolve, reject) => fs.access(p, err => {
                if (err)
                    return fs.mkdir(p, { mode: inputStats.mode }, (err) => {
                        if (err)
                            return reject(err);
                        resolve();
                        mods.directory[0]++;
...
function warmUp() {
    ...
    const cb = (err) => {
        if (err)
            log.debug(err), log.error('Something went wrong while warming up');
        cpc.sendJob('ready', null);
    }, path = PATH.join(appDataPath, 'temp_' + process.env.workerId);
    try {
        fs.writeFile(path, Buffer.concat([crypto.randomBytes(5242880), crypto.randomBytes(5242880)]), (err) => {
            if (err)
                return cb(err);
            fs.createReadStream(path)
                .pipe(crypto.createCipheriv('aes-256-ctr', crypto.randomBytes(32), hashMD5(crypto.randomBytes(32).toString('hex'))))
                .pipe(fs.createWriteStream(path + '.warmup'))
                .on('finish', () => fs.rename(path + '.warmup', path, (err) => {
                if (err)
                    log.debug(err), log.error('Something went wrong while warming up');
                fs.unlink(path, cb);
            }));
