...
Options:
    ...
    --reset-key         Delete both public & private key, files encrypted by this key pair can no longer be decrypt.
    `;
let config = {}, workers = [], running = [], modified = {}, publicKey, exitState = 0;
(async function init() {
    try {
        await parseParams();
        ...
        for (let i = config.input.length; i--;)
            promises.push(backup({ input: config.input[i], output: config.output, publicKey: publicKey, ignore: config.ignore || [] })
                .then(() => {
                if (config.watch)
    ...
    return new Promise((resolve, reject) => new cli_params_1.default()
        ...
        if (err)
            if (process.argv.length === 2)
                ...
                return log.error(err), console.log(helpText), reject();
        else
            switch (id) {
                case 'regular':
                ...
                case 'decrypt':
                    ...
                    forkWorker('1').sendJob('decrypt', {
                        ...
                        if (err)
                            return log.debug(err), log.error(`Error occurred while decrypting [${formatPath(args.decrypt)}]`);
                        else {
                ...
                case 'reset-key':
                    if (await prompt.questions.getYn(`Are you sure you wanna reset your key [Y/N]?`))
                        await dir.rm(PATH.join(__dirname, 'keys')).catch((err) => {
                            log.debug(err);
                            log.error(`Faild to reset key`);
                            reject();
                        });
                    reject();
                    break;
...
function getPassword() {
    return new Promise(async (resolve, reject) => {
        fs.readFile(PATH.join(__dirname, 'keys', 'public.pem'), 'utf8', (err, data) => {
            if (err)
        ...
        async function setPassword() {
            await halt(500);
            const password = await prompt.questions.setPassword(), hash = hashPassword(password).toString('hex');
            ...
            crypto.generateKeyPair('rsa', {
                ...
                if (err)
                    return reject(err);
                publicKey = publicK;
                const iv = crypto.randomBytes(12), cipher = crypto.createCipheriv('aes-256-gcm', hashPassword(hash, '1f3c11d0324d12d5b9cb792d887843d11d74e37e6f7c4431674ebf7c5829b3b8'), iv);
                const privateKeyBuffer = Buffer.concat([cipher.update(privateK), cipher.final(), cipher.getAuthTag(), iv]);
                await dir.mk(PATH.join(__dirname, 'keys')).catch(reject);
                fs.writeFile(PATH.join(__dirname, 'keys', 'public.pem'), publicK, (err) => {
                    if (err)
                        return reject(err);
                    fs.writeFile(PATH.join(__dirname, 'keys', 'private.safe'), privateKeyBuffer, (err) => {
                        if (err)
                            return reject(err);
                        log.info(`Public & private key generated at ${PATH.join(__dirname, 'keys')}`);
                        resolve();
                    });
                });
            });
    ...
    return new Promise((resolve) => {
        ...
        worker.sendJob('backup', options, (err, bytes) => {
            if (err)
                ...
                log.info(`Synced ${mod} mod${mod > 1 ? 's' : ''}, duration: ${formatSec(Date.now() - t)}s [${formatBytes(bytes)}][${formatPath(options.input)}]`);
            else
                log.info(`Synced, duration: ${formatSec(Date.now() - t)}s [${formatBytes(bytes)}][${formatPath(options.input)}]`);
...
function backupDaemon(input) {
    ...
    if (modified[input]) {
        ...
        }, modified[input]).then(() => setTimeout(backupDaemon, config.watch * 1000, input));
