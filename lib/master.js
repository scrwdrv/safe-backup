...
Options:
    ...
    `;
let config = {}, workers = [], running = [], modified = {}, keys = {}, exitState = 0, test;
(async function init() {
    try {
        let pkg;
        ...
        function checkPath() {
            for (let typeOfPath of ['input', 'output'])
                for (let i = config[typeOfPath].length; i--;) {
                    if (typeOfPath === 'input')
                        normalizeInput(config[typeOfPath][i], (type, p) => {
...
function forkWorkers(count = 0) {
    return new Promise(resolve => {
        cpc.onWorker('ready', () => {
            count++;
            if (count === physical_cores_1.default)
                resolve();
        });
...
function forkWorker(id, init = true) {
...
async function runTest(n = 0, d = 0) {
    ...
    if (n === test)
        return log.warn(`Test finished, avg time: ${Math.round(d / test)}ms`), exit();
    const t = Date.now();
...
function backup(options, watch = false) {
    return new Promise(resolve => {
        const worker = getWokrer(), t = Date.now();
        running.push(worker.id);
        normalizeInput(options.input, (type, p) => {
            worker.sendJob(type === 0 ? 'backup' : 'plainBackup', { ...options, input: p }, (err, bytes, mods) => {
                const tDiff = Date.now() - t, index = running.indexOf(worker.id);
                if (err)
                    log.debug(err), log.error(`Error occurred while syncing [${formatPath(p)}]`),
                        log.warn(`If this happens continuously, try to delete old backup file`);
                else
                    log.info(`Synced ${type === 0 ? '& encrypted ' : ''}[${formatSec(tDiff)}s][${formatBytes(bytes)}][${(bytes * options.output.length / 1048576 / (tDiff / 1000)).toFixed(2)} MBps][F:(+${mods.file[0]})(-${mods.file[1]})][D:(+${mods.directory[0]})(-${mods.directory[1]})][${formatPath(p)}]`);
                if (index > -1)
                    running.splice(index, 1);
                if (watch)
                    fs.stat(p, (err, stats) => {
...
function watchMod(path, isFile, retry = 0) {
    normalizeInput(path, (type, p) => {
        ...
        const watcher = node_watch_1.default(p, { recursive: !isFile }, (evt, file) => {
...
function normalizeInput(path, cb) {
    if (/^\*/.test(path))
        return cb(1, path.slice(1));
    cb(0, path);
}
