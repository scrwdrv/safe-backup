                ...
                if (err)
                    ...
                    case 'decrypt':
                        ...
                        forkWorker('1').sendJob('decrypt', {
                            ...
                            if (err) return log.debug(err), log.error(`Error occurred while decrypting [${formatPath(args.decrypt)}]`);
                            else {
...
function getPassword() {
    return new Promise<void>(async (resolve, reject) => {
        ...
        async function setPassword() {
            await halt(500);
            const password = await prompt.questions.setPassword(),
                hash = hashPassword(password).toString('hex');
