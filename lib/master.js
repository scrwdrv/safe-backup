...
Options:
    ...
    -I --ignore         Add ignore rule(s) with regex, separate by space.  
    ...
    try {
        await new Promise((resolve, reject) => {
            fs.readFile(PATH.join(__dirname, '../', 'package.json'), 'utf8', async (err, data) => {
                ...
                await startup_update_check_1.default({ name: pkg.name, version: pkg.version }).catch((err) => {
        ...
        await getPassword();
        const newerVersion = await startup_update_check_1.default(PATH.join(__dirname, '../', 'package.json')).catch((err) => {
            log.warn(`Failed to check for updates with npm`);
        });
        if (newerVersion)
            ...
            switch (id) {
                ...
                case 'import-key':
                    fs.readFile(args['import-key'], (err, data) => {
                        if (err)
                            return log.debug(err), console.log(`Key pair not found at ${PATH.resolve(args['import-key'])}`), reject();
                        try {
                ...
                case 'export-config':
                    fs.readFile(PATH.join(appDataPath, 'config.json'), 'utf8', (err, data) => {
                        if (err)
                            return log.debug(err), console.log('Configuration not found'), reject();
                ...
                case 'import-config':
                    fs.readFile(args['import-config'], 'utf8', (err, data) => {
                        if (err)
                            return log.debug(err), console.log(`Configuration not found at ${PATH.resolve(args['import-config'])}`), reject();
                        try {
