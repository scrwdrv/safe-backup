...
cpc.onMaster('decrypt', async (req, res) => {
    ...
    try {
        const l = req.output.length, name = formatPath(req.input.replace(/[\\*/!|:?<>]+/g, '-'), 255) + '.backup', key = crypto.randomBytes(32), iv = crypto.randomBytes(12), cipher = crypto.createCipheriv('aes-256-gcm', key, iv), buffers = [Buffer.from(isFile ? 'F' : 'D'), iv, crypto.publicEncrypt(req.publicKey, key), Buffer.from(req.privateKey, 'hex')], writeStream = new stream_1.Writable({
        ...
        writeStream.write(Buffer.concat([
            ...
            if (err)
                return res(err), writeStream.destroy();
            if (isFile)
                fs.createReadStream(req.input).pipe(cipher).pipe(writeStream);
            else
                tar.pack(req.input, {
                    ignore: (file) => {
